...
# Monkey-patch requests and httpx
...
def log_response(response: AgentOutput) -> None:
	...
	# st.write((f'{emoji} Eval: {response.current_state.evaluation_previous_goal}'))
	logger.info(f'🧠 Memory: {response.current_state.memory}')
	# st.write(f'🧠 Memory: {response.current_state.memory}')
	logger.info(f'🎯 Next goal: {response.current_state.next_goal}')
	# st.write(f'🎯 Next goal: {response.current_state.next_goal}')
	for i, action in enumerate(response.action):
		logger.info(f'🛠️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
		# st.write(f'🛠️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
		output_text = str(f'🛠️  Action {i + 1}/{len(response.action)}: {action.model_dump_json(exclude_unset=True)}')
		prompt_template = """
		Given the following output from a task completion, extract the headline and summary. 

		Output:
		{output_text}

		Return the extracted headline, content of the article and summary in the format below:

		Headline: [EXTRACTED HEADLINE]
		Content : [EXTRACTED CONTENT]
		Summary: [EXTRACTED SUMMARY]

		Make sure to only extract the relevant headline and summary from the text.
		"""
		prompt = PromptTemplate(input_variables=["output_text"], template=prompt_template)
		try:
    # Extract the JSON portion of the string
			json_str_match = re.search(r'\{.*\}', output_text)
			# st.write(json_str_match)
			if json_str_match:
				json_str = json_str_match.group(0)
				# st.write(json_str)

				# Parse JSON to check if "done" key exists
				parsed = json.loads(json_str)
				# st.write(parsed)
				if "done" in parsed:
					# st.write('Done is in parsed and parsed is..')
					# st.write(parsed)
					# Proceed with sending the formatted prompt to LLM if "done" exists
					llm_chain = LLMChain(prompt=prompt, llm=llm)
					output_text = parsed['done']['text']
					response = llm_chain.invoke({"output_text": output_text})
					# st.write(f'response 1 is..')
					# st.write(response)
					response = response['text']
					st.write(f'response 2 is..')
					st.write(response)

					# Print the result
					# st.write(response["choices"][0]["text"].strip())
				else:
					print("❌ No 'done' field in the output text.")
			else:
				print("❌ Unable to extract JSON from output text.")
...
class Agent(Generic[Context]):
	...
	def __init__(
		...
		logger.info('Done with _set_message_context()')
	...
	def _set_model_names(self) -> None:
		...
		logger.info('I am in function _set_model_names')
	...
	def _set_tool_calling_method(self) -> Optional[ToolCallingMethod]:
		...
		logger.info('self.chat_model_library is..')
		logger.info(self.chat_model_library)
		if tool_calling_method == 'auto':
			...
			# for chatgroq
				...
				logger.info(' Going to return json mode for Chatgroq')
				return 'json_mode'
	...
	async def get_next_action(self, input_messages: list[BaseMessage]) -> AgentOutput:
		...
		logger.warning('self.tool_calling_method is..')
		logger.warning(self.tool_calling_method)
